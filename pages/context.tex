    ...
    \begin{tikzpicture}[overlay,remember picture]
        ...
        \onslide<+-> {
            \node[align=center,text width=0.55\textwidth] (problem) at ($(sparse.south)+(0,-0.15\textheight)$) {%
                \begin{blockcolor}{ColorOne}{Problem of interest}
                    \centering
                    $\opt{\pv} \in \textstyle\argmin_{x} \ \lossfunc(\dic\pv) + \underbrace{\lambda \norm{x}{1} + \smoothfunc(\pv)}_{\regfunc(\pv)}$
                \end{blockcolor}
            };
        }
        %
        ...
        \onslide<+-> {
            \node[align=left,text width=\textwidth] at ($(problem.south)+(0,-0.225\textheight)$) {%
                ...
                \begin{itemize}
                    \item $\lossfunc(\cdot)$ and $\smoothfunc(\cdot)$ are \emphcolor{ColorOne}{proper}, \emphcolor{ColorOne}{closed} and \emphcolor{ColorOne}{convex} functions
                    \item $\lossfunc(\cdot)$ and $\smoothfunc(\cdot)$ are \emphcolor{ColorOne}{differentiable} with \emphcolor{ColorOne}{Lipschitz-continuous} gradient
                    \item $\smoothfunc(\cdot)$ is \emphcolor{ColorOne}{separable}
                    \begin{itemize}
                        \item[$\rightarrow$] \scriptsize{With additional mild assumptions ensuring non-degeneracy of the problem}
                    \end{itemize}
                \end{itemize}
            };
        }
        ...
        \onslide<+-> {
            ...
            \node[align=left,text width=\textwidth] (methods) at ($(current page.north)+(0,-0.45\textheight)$) {%
                ...
                \begin{itemize}
                    ...
                    \item \emphcolor{ColorOne}{First-order} methods accessing $\grad\lossfunc(\cdot)$, $\subdiff\regfunc(\cdot)$, $\prox{\lossfunc/\regfunc}{\cdot}$, ...
                    \begin{itemize}
