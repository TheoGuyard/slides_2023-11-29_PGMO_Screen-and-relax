        ...
        \onslide<+-> {
            \node[align=center,text width=0.6\linewidth] (tests) at ($(current page.north)+(0,-0.8\textheight)$) {%
                \begin{blockcolor}{black}{Geometrical screening and relaxing test}
                    \centering
                    $\begin{array}{ll}
                        \subdiff_{\idxentry}\regfunc(\opt{\pv}) \subset [-\reg,\reg] &\iff \ \opt{\pvi{\idxentry}} = 0 \\
                        \subdiff_{\idxentry}\regfunc(\opt{\pv}) \not\subset [-\reg,\reg] &\iff \ \opt{\pvi{\idxentry}} \neq 0 
                    \end{array}$
                \end{blockcolor}
            };
        }
        ...
        \onslide<+-> {
            \node[align=center,text width=0.55\linewidth] (dual) at ($(current page.north)+(0,-0.35\textheight)$) {%
                \begin{blockcolor}{black}{Dual problem}
                    \centering
                    $\opt{\dv} \in \textstyle\argmax_{\dv} \ -\conj{\lossfunc}(-\dv) - \conj{\regfunc}(\transp{\dic}\dv)$
                \end{blockcolor}
            };
        ...
        \onslide<+-> {
            \node[align=center,text width=0.5\linewidth] (tests) at ($(optcond)+(0,-0.35\textheight)$) {%
                \begin{blockcolor}{black}{Screening and relaxing test}
                    \centering
                    $\begin{array}{rcl}
                        \abs{\transp{\atom{\idxentry}}\opt{\dv}} \leq \reg &\iff& \opt{\pvi{\idxentry}} = 0 \\
                        \abs{\transp{\atom{\idxentry}}\opt{\dv}} > \reg &\iff& \opt{\pvi{\idxentry}} \neq 0
                    \end{array}$
                \end{blockcolor}
            };
        }
...
\begin{frame}{Safe regions}
    \begin{tikzpicture}[overlay,remember picture]
        ...
        \onslide<+-> {
            \node[align=center,text width=0.75\linewidth] (safetests) at ($(current page.north)+(0,-0.50\textheight)$) {%
                \begin{blockcolor}{black}{Safe screening and relaxing test}
                    \centering
                    $\begin{array}{rcl}
                        \max_{\emphcolor{ColorOne}{\dv \in \saferegion}}\abs{\transp{\atom{\idxentry}}\dv} \leq \reg &\implies& \opt{\pvi{\idxentry}} = 0 \\
        ...
        %
        \onslide<+-> {
            \node[text width=0.3\textwidth,align=center,draw,dashed,ultra thick,fill=gray!15,font=\scriptsize] at (8.5,-2.75) {
                \emphcolor{ColorOne}{Lasso problem} \\ Limit case with $\smoothfunc(\pv) = 0$
            };
        }
    \end{tikzpicture}
\end{frame}
